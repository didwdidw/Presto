"""
simple pipelien for demostrating presto
Weiwei Zhu
2015-08-14
...
filename = "Dec+1554_arcdrift+23.4-M12_0194.fil"
if len(sys.argv) > 2:
    maskfile = sys.argv[2]
else:
    maskfile = None
...
output = subprocess.getoutput(readheadercmd)
print(output)
output = str(output)
header = {}
...
try:
    ...
    ddplancmd = 'python3 ~/ASC/presto/bin/DDplan.py -d %(maxDM)s -n %(Nchan)d -b %(BandWidth)s -t %(tsamp)f -f %(fcenter)f -s %(Nsub)s -o DDplan.ps' % {
...
try:
    ...
    for line in ddplan:
        ...
        for i, dml in enumerate(dmlist):
            ...
            print(prepsubband)
            output = subprocess.getoutput(prepsubband)
            ...
            print(prepsubcmd)
            output = subprocess.getoutput(prepsubcmd)
            logfile.write(output)
    os.system('rm *.sub*')
    logfile.close()
    os.chdir(cwd)
...
try:
    ...
    for df in datfiles:
        fftcmd = "realfft %s" % df
        print(fftcmd)
        output = subprocess.getoutput(fftcmd)
        logfile.write(output)
    logfile.close()
    ...
    for fftf in fftfiles:
        ...
        print(searchcmd)
        output = subprocess.getoutput(searchcmd)
        logfile.write(output)
    logfile.close()
    os.chdir(cwd)
except:
...
cwd = os.getcwd()
os.chdir('subbands')
subprocess.getoutput('ln -s ../%s %s' % (filename, filename))
...
for cand in cands:
    ...
    print(foldcmd)
        #os.system(foldcmd)
    output = subprocess.getoutput(foldcmd)
    logfile.write(output)
logfile.close()
os.chdir(cwd)
#except:
